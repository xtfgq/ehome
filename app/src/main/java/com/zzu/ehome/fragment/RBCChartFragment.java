...
import com.zzu.ehome.adapter.MaiboResChatAdapter;
 ...
 */
public class RBCChartFragment extends BaseFragment implements View.OnClickListener,StickyListHeadersListView.OnHeaderClickListener {
    private View mView;
...
//        cardNo="410223198012031258";
    ...
    public void initViews() {
        ...
        heardchat = (LinearLayout) inflater.inflate(R.layout.new_rbc_layout, null);
    ...
    private void getTop(int num) {
        requestMaker.BloodRoutineInquiryForLine(cardNo,  "", "",  num + "", "RBC",new JsonAsyncTask_Info(getActivity(), true, new JsonAsyncTaskOnComplete() {
            @Override
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.rl_weight_time && pvTime != null) {
            ...
            getRBCRecords(tvstart.getText().toString(),tvend.getText().toString());
        }
    ...
    private void getRBCRecords(String starttime,String endtime){
        ...
        try {
            ...
            if (dtend.getTime() >= dtStart.getTime()) {
                requestMaker.BloodRoutineInquiryForLine(cardNo,starttime,endtime,"", "RBC",new JsonAsyncTask_Info(getActivity(), true, new JsonAsyncTaskOnComplete(){
                    @Override
                    public void processJsonObject(Object result) {
