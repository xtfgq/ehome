...
import com.zzu.ehome.adapter.WeightResAdapter;
 ...
 */
public class TizhongChartFragment extends BaseFragment implements View.OnClickListener,StickyListHeadersListView.OnHeaderClickListener {
    private View mView;
    ...
    private WeightResAdapter mAadpter;
    ...
    public static Fragment getInstance() {
        return new TizhongChartFragment();
    }

    public void initViews() {
        inflater = LayoutInflater.from(getActivity());
        mAadpter = new WeightResAdapter(getActivity());
    ...
    private void getTop(int num) {
        requestMaker.WeightInquiryType(cardNo, userid, "", "", "T", num + "", new JsonAsyncTask_Info(getActivity(), true, new JsonAsyncTaskOnComplete() {
            @Override
                    ...
                    if (array.getJSONObject(0).has("MessageCode")) {
                        ...
                        if(array.length()>=6){
                            ...
                            for (i = list.size() - 1, j = 0; i >= 0; i--, j++) {
                                linePoint.add(new PointD(Double.valueOf(j), Double.valueOf(list.get(i).getWeight())));
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.rl_weight_time && pvTime != null) {
            ...
            getWeightRecords(tvstart.getText().toString(),tvend.getText().toString());
        }
    ...
    private void getWeightRecords(String starttime,String endtime) {
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        Date dtStart;
        Date dtend ;
        ...
        try {
            ...
            if (dtend.getTime() >= dtStart.getTime()) {
                requestMaker.WeightInquiryType(cardNo, userid, starttime, endtime, "", "", new JsonAsyncTask_Info(getActivity(), true, new JsonAsyncTaskOnComplete() {
                    @Override
                    public void processJsonObject(Object result) {
                        ...
                        try {
                            ...
                            if (array.getJSONObject(0).has("MessageCode")) {
                                ...
                                List<WeightRes> list = new ArrayList<WeightRes>();
                                labels.clear();
                                mChart.setX(labels, list.size());
