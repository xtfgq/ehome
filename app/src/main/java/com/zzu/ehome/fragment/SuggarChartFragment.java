 ...
 */
public class SuggarChartFragment extends BaseFragment implements View.OnClickListener, StickyListHeadersListView.OnHeaderClickListener {
    private View mView;
    ...
    private SuggarChatAdapter mAadpter;
...
//     cardNo="410223198009061208";
    ...
    public static Fragment getInstance(String str) {
        title = str;
        return new SuggarChartFragment();
    }

    public void initViews() {
        inflater = LayoutInflater.from(getActivity());
        mAadpter = new SuggarChatAdapter(getActivity());
    ...
    private void getTop(int num) {
        requestMaker.BiochemistryInquiryForLine(cardNo, "GLU", "", "", num + "", new JsonAsyncTask_Info(getActivity(), true, new JsonAsyncTaskOnComplete() {
            @Override
            public void processJsonObject(Object result) {
                ...
                try {
                    ...
                    JSONArray array = mySO
                            .getJSONArray("BiochemistryInquiryForLine");
                    if (array.getJSONObject(0).has("MessageCode")) {
                        layout_no_msg.setVisibility(View.VISIBLE);
                    } else {
                        layout_no_msg.setVisibility(View.GONE);
                        listview.setVisibility(View.VISIBLE);
                        SuggarXueDate data = JsonTools.getData(result.toString(), SuggarXueDate.class);
                        List<SuggarXueBean> list = data.getData();
                        ...
                        if(array.length()>=6){
                            ...
                            for (i = list.size() - 1, j = 0; i >= 0; i--, j++) {
                                ...
                                //0餐前，1餐后，2随机
                                if (Integer.valueOf(list.get(i).getHoursAfterMeal()) == 0) {
                                    linePoint2.add(new PointD(xd, yd));
                                } else if (Integer.valueOf(list.get(i).getHoursAfterMeal()) == 1) {
                                    linePoint1.add(new PointD(xd, yd));
                                } else {
                                    linePoint3.add(new PointD(xd, yd));
                                }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.rl_weight_time && pvTime != null) {
            ...
            getSuggarRecords(tvstart.getText().toString(), tvend.getText().toString());
        }
    ...
    private void getSuggarRecords(String starttime, String endtime) {
        ...
        try {
            ...
            if (dtend.getTime() >= dtStart.getTime()) {
                requestMaker.BiochemistryInquiryForLine(cardNo, "GLU", starttime, endtime, "", new JsonAsyncTask_Info(getActivity(), true, new JsonAsyncTaskOnComplete() {
                    @Override
                    public void processJsonObject(Object result) {
                        ...
                        try {
                            ...
                            if (array.getJSONObject(0).has("MessageCode")) {
                                ...
                                labels.clear();
                                List<SuggarXueBean> list = new ArrayList<SuggarXueBean>();
                                List<PointD> linePoint1 = new ArrayList<PointD>();
                                List<PointD> linePoint2 = new ArrayList<PointD>();
                                List<PointD> linePoint3 = new ArrayList<PointD>();
                                mChart.setX(labels, list.size());
