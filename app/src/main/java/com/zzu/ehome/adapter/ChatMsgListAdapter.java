...
import com.zzu.ehome.bean.ChatEntity;
import com.zzu.ehome.view.CustomTextView;
...
public class ChatMsgListAdapter extends BaseAdapter implements AbsListView.OnScrollListener {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (item.getType() != Constants.TEXT_TYPE) {
            ...
            holder.sendContext.setBackgroundResource(R.drawable.img_chat_black);
            holder.sendContext.setTextColor(context.getResources().getColor(R.color.white));
        } else {
            // 根据名称计算颜色
            holder.sendContext.setBackgroundResource(R.drawable.img_chat_white);
...
//            continueAllAnimator();
//        }
        if (listMessage.size() >= MAXITEMCOUNT) {
            continueAllAnimator();
        }
