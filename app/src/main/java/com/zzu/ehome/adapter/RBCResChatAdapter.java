 ...
 */
public class RBCResChatAdapter extends BaseAdapter implements StickyListHeadersAdapter {
    public List<RBCBean> getList() {
        return list;
    }

    public void setList(List<RBCBean> list) {
        this.list = list;
    }
    ...
    public RBCResChatAdapter(Context context, String title) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        RBCBean res = (RBCBean) getItem(position);
        if (convertView == null) {
        ...
        String mb = title + ":  " + decimalFormat.format(Double.valueOf(res.getItemValue())) +"  "+ res.getItemUnit();
        SpannableString style1 = new SpannableString(mb);
        style1.setSpan(
                new TextAppearanceSpan(mContext, R.style.styleNormalColor), title.length() + 1, mb.length()-res.getItemUnit().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        holder.tv_ssy.setText(style1);
        float value = Float.parseFloat(res.getItemValue());
        float max = Float.parseFloat(res.getItemMaxValue());
        float min = Float.parseFloat(res.getItemMinValue());
