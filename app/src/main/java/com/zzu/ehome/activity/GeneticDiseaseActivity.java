 ...
 */
public class GeneticDiseaseActivity extends BaseActivity {
    private ListView listview;
    private ContainsEmojiEditText editText;
    private Button btn_save;
    private List<DiseaseBean> mList;
    private SwitchAdapter adapter;
    private String geneticdisease = null;

    @Override
    protected void onCreate(Bundle arg0) {
        super.onCreate(arg0);
        setContentView(R.layout.layout_medicine);
        geneticdisease = getIntent().getStringExtra("geneticdisease");
        initViews();
        initEvent();
        initDatas();
    }
    ...
    public void initViews() {
        ...
        setLeftWithTitleViewMethod(R.mipmap.icon_arrow_left, "遗传病史", new HeadView.OnLeftClickListener() {
            @Override
            public void onClick() {
    ...
    public void doAdd() {
        ...
        if (!TextUtils.isEmpty(name)) {
            if (!IOUtils.isName(name)) {
                ToastUtils.showMessage(GeneticDiseaseActivity.this, "请输入中文名称");
                return;
            }

            sbf.append(name);
        }
        intent.putExtra("geneticdisease", sbf.toString());
        setResult(BaseFilesActivity.GENETICDISEASE, intent);
        finish();
    }
}
