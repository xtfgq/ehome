...
import com.zzu.ehome.utils.DisplayUtil;
...
public class CircleBar extends View {
    ...
    private void init() {
        ...
        circleStrokeWidth = DisplayUtil.dip2px(getContext(), 16);// 圆圈的线条粗细
        mTextSize = DisplayUtil.sp2px(getContext(), 48);// 中间文字大小
        mTextSize2 = DisplayUtil.sp2px(getContext(), 50);
        mTextSize1 = DisplayUtil.sp2px(getContext(), 20);// 上下文字大小
        mDistance = DisplayUtil.dip2px(getContext(), 46);// 文字间的距离
        mDistance1 = DisplayUtil.dip2px(getContext(), 12);//圆圈离父布局的距离

        // 绘制蓝色扇形的画笔
        mColorWheelPaint = new Paint();
        mColorWheelPaint.setAntiAlias(true);// 抗锯齿
        mColorWheelPaint.setColor(0xFF30C0CB);// 设置颜色
